...
import {
  ...
  ShieldCheck, Target, Trash2, TrendingUp, UserCog, Users, X,
} from 'lucide-react'
...
function AdminProjectDetailPage({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  const save = async () => { try { setMessage('Saving…'); await adminRequest(`/api/admin/projects/${projectId}`, { method: 'PATCH', body: JSON.stringify({ dashboard: { imageUrl: draft.image_url, userGoal: draft.user_goal, costBudget: draft.cost_budget, costActual: draft.cost_actual, adoptionRate: draft.adoption_rate, forecastPenetration: draft.forecast_penetration, milestones: draft.milestones, tasks: draft.tasks } }) }); setProject(draft); setEditing(false); setMessage('Project dashboard saved.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not save the project dashboard.') } }
  ...
  return <div className="project-detail">
    ...
    <section className="project-identity" id="overview"><div className="project-cover">{draft.image_url?<div className="project-cover-image" role="img" aria-label={`${project.name} project`} style={{backgroundImage:`url(${draft.image_url})`}}/>:<span><ImageIcon/><b>{initials}</b></span>}</div><div>{editing?<><label>Project image URL<input value={draft.image_url} onChange={event=>setDraft({...draft,image_url:event.target.value})}/></label><label>Title<input value={draft.name} disabled/></label><label>Brief<textarea value={draft.description} disabled/></label></>:<><h1>{project.name}</h1><p>{project.description||'No project brief has been added.'}</p></>}</div></section>
