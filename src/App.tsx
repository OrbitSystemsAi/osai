...
function ProjectPitchLinks({ active, onSelect, sections = projectPitchSections }: { active?: string; onSelect?: (section: string) => void; sections?: readonly string[] }) { return <nav className="project-pitch-links" aria-label="Project presentation sections">{sections.map(label=><a className={active===label?'active':undefined} key={label} href={`#${label.toLowerCase().replaceAll(' ','-')}`} onClick={onSelect?event=>{event.preventDefault();onSelect(label)}:undefined}>{label}</a>)}</nav> }
...
function PulsePage() {
  ...
  const editionMonthNumber = new Intl.DateTimeFormat('en-US', { month: '2-digit', timeZone: 'America/New_York' }).format(today)
  ...
  return <><PageHead title="Today’s Pulse" intro="" />
    <div className="pulse-edition-bar">
      <time dateTime={`${editionYear}-${editionMonthNumber}-${editionDay}`}>{editionYear} Edition: {editionMonth} {editionDay}</time>
...
function ProjectsPage({ isAdmin }: { isAdmin: boolean }) {
  ...
  const openAdminProject = (project: CatalogProject) => { if (!project.id) return; window.history.pushState({},'',`/member/projects?adminEdit=${project.id}&edit=overview#overview`); setAdminEditingProjectId(project.id); window.scrollTo(0,0) }
...
function projectDetailFromApi(project: AdminProjectDetail): AdminProjectDetail {
  return { ...project, user_goal: Number(project.user_goal), user_actual: Number(project.user_actual), cost_budget: Number(project.cost_budget), cost_actual: Number(project.cost_actual), adoption_rate: Number(project.adoption_rate), forecast_penetration: Number(project.forecast_penetration), milestones: project.milestones || [], tasks: project.tasks || [], problem_content:project.problem_content || [], solution_content:project.solution_content || [], competition_content:project.competition_content || [], market_content:project.market_content || [], business_model_content:project.business_model_content || [] }
}

function storyContentPayload(project: AdminProjectDetail) {
  return { problemContent:project.problem_content, solutionContent:project.solution_content, competitionContent:project.competition_content, marketContent:project.market_content, businessModelContent:project.business_model_content }
}
...
function ProjectStoryPageEditor({ section, blocks, onSave }: { section: StorySection; blocks: StoryBlock[]; onSave: (blocks: StoryBlock[]) => Promise<void> }) {
  ...
  const sectionDefaults: Record<StorySection, { title: string; description: string }> = {
    Problem: { title: 'Tell the story of the problem.', description: 'Add a description of what the problem is that this product is solving.' },
    Solution: { title: 'Explain the solution.', description: 'Add a description of how this product solves the problem.' },
    Competition: { title: 'Competition', description: 'Describe the competitive landscape and how this project is differentiated.' },
    Market: { title: 'Market', description: 'Describe the target market, audience, and opportunity.' },
    'Business Model': { title: 'Business Model', description: 'Explain how this project creates, delivers, and captures value.' },
  }
  ...
  const nextId = (type: StoryBlock['type']) => { let index=content.length+1; while(content.some(block=>block.id===`${sectionKey}-${type}-${index}`)) index+=1; return `${sectionKey}-${type}-${index}` }
  const commit = async (next: StoryBlock[]) => { setContent(next); setMessage('Saving…'); try { await onSave(next); setMessage('') } catch (error) { setMessage(error instanceof Error?error.message:`Could not save the ${section} page.`) } }
  const saveIntro = async () => { const titleId=`${sectionKey}-title`; const descriptionId=`${sectionKey}-description`; await commit([{id:titleId,rowId:titleId,type:'heading',text:introTitle.trim()||defaultTitle},{id:descriptionId,rowId:descriptionId,type:'paragraph',text:introDescription.trim()||defaultDescription}]);setEditingIntro(null) }
  const textBlock = (type: Exclude<StoryBlock['type'],'image'>, rowId?:string):StoryBlock => { const labels:Record<Exclude<StoryBlock['type'],'image'>,string>={heading:'Section heading',paragraph:`Describe the ${section.toLowerCase()}…`,quote:'Add a supporting quote…',list:'Add one item per line…',statistic:'Add a key statistic…'}; const id=nextId(type); return {id,rowId:rowId||id,type,text:labels[type]} }
  const addTextBlock = (type: Exclude<StoryBlock['type'],'image'>, index: number) => { const block=textBlock(type); const next=[...content.slice(0,index),block,...content.slice(index)]; setContent(next); setEditingId(block.id) }
  const insertTextBeside = (type:Exclude<StoryBlock['type'],'image'>, block:StoryBlock, side:'left'|'right') => { const rowId=block.rowId||block.id; const index=content.findIndex(item=>item.id===block.id); const sibling=textBlock(type,rowId); const normalized=content.map(item=>item.id===block.id?{...item,rowId}:item); const insertion=index+(side==='right'?1:0); const next=[...normalized.slice(0,insertion),sibling,...normalized.slice(insertion)]; setContent(next);setEditingId(sibling.id) }
  ...
  const addImageBlock = async (event: ChangeEvent<HTMLInputElement>, index: number) => { const upload=await readImage(event);if(!upload)return;const id=nextId('image');const block:StoryBlock={id,rowId:id,type:'image',imageUrl:upload.imageUrl,caption:'',alt:upload.file.name.replace(/\.[^.]+$/,'')};const next=[...content.slice(0,index),block,...content.slice(index)];setEditingId(block.id);await commit(next) }
  const insertImageBeside = async (event:ChangeEvent<HTMLInputElement>,block:StoryBlock,side:'left'|'right') => { const upload=await readImage(event);if(!upload)return;const rowId=block.rowId||block.id;const index=content.findIndex(item=>item.id===block.id);const id=nextId('image');const sibling:StoryBlock={id,rowId,type:'image',imageUrl:upload.imageUrl,caption:'',alt:upload.file.name.replace(/\.[^.]+$/,'')};const normalized=content.map(item=>item.id===block.id?{...item,rowId}:item);const insertion=index+(side==='right'?1:0);const next=[...normalized.slice(0,insertion),sibling,...normalized.slice(insertion)];setEditingId(id);await commit(next) }
  ...
  const rows = content.reduce<StoryBlock[][]>((result,block)=>{const current=result[result.length-1];if(current&&rowKey(current[0])===rowKey(block))current.push(block);else result.push([block]);return result},[])
...
function AdminProjectDetailPage({ projectId, onBack, onProjectSaved }: { projectId: string; onBack: () => void; onProjectSaved?: (project: AdminProjectDetail) => void }) {
  ...
  const save = async () => { try { setMessage('Saving…'); const normalized = { ...draft, name: (draft.name.trim() || 'Project Title').slice(0, PROJECT_TITLE_MAX), description: draft.description.trim().slice(0, PROJECT_DESCRIPTION_MAX) }; await adminRequest(`/api/admin/projects/${projectId}`, { method: 'PATCH', body: JSON.stringify({ name: normalized.name, slug: normalized.slug, description: normalized.description, status: normalized.status, accessLevel: normalized.access_level }) }); await adminRequest(`/api/admin/projects/${projectId}`, { method: 'PATCH', body: JSON.stringify({ dashboard: { imageUrl: normalized.image_url, userGoal: normalized.user_goal, costBudget: normalized.cost_budget, costActual: normalized.cost_actual, adoptionRate: normalized.adoption_rate, forecastPenetration: normalized.forecast_penetration, milestones: normalized.milestones, tasks: normalized.tasks, ...storyContentPayload(normalized) } }) }); setDraft(normalized); setProject(normalized); onProjectSaved?.(normalized); setEditingPart(null); setMessage('') } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not save the project dashboard.') } }
  const saveProjectImage = async (imageUrl: string) => { try { setMessage('Saving…'); const next = { ...draft, image_url:imageUrl }; await adminRequest(`/api/admin/projects/${projectId}`, { method:'PATCH', body:JSON.stringify({ dashboard:{ imageUrl, userGoal:next.user_goal, costBudget:next.cost_budget, costActual:next.cost_actual, adoptionRate:next.adoption_rate, forecastPenetration:next.forecast_penetration, milestones:next.milestones, tasks:next.tasks, ...storyContentPayload(next) } }) }); setDraft(next); setProject(next); onProjectSaved?.(next); setEditingPart(null); setMessage('') } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not save the project image.') } }
  const saveTimeline = async (milestones: Milestone[]) => { try { const ordered = [...milestones].sort((a,b)=>a.date.localeCompare(b.date)); const next = { ...draft, milestones:ordered }; setDraft(next); await adminRequest(`/api/admin/projects/${projectId}`, { method:'PATCH', body:JSON.stringify({ dashboard:{ imageUrl:next.image_url, userGoal:next.user_goal, costBudget:next.cost_budget, costActual:next.cost_actual, adoptionRate:next.adoption_rate, forecastPenetration:next.forecast_penetration, milestones:ordered, tasks:next.tasks, ...storyContentPayload(next) } }) }); setProject(next); onProjectSaved?.(next); setMessage('') } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not save the project timeline.') } }
  const saveStoryContent = async (field: 'problem_content'|'solution_content'|'competition_content'|'market_content'|'business_model_content', content: StoryBlock[]) => { const next={...draft,[field]:content}; await adminRequest(`/api/admin/projects/${projectId}`,{method:'PATCH',body:JSON.stringify({dashboard:{imageUrl:next.image_url,userGoal:next.user_goal,costBudget:next.cost_budget,costActual:next.cost_actual,adoptionRate:next.adoption_rate,forecastPenetration:next.forecast_penetration,milestones:next.milestones,tasks:next.tasks,...storyContentPayload(next)}})}); setDraft(next);setProject(next);onProjectSaved?.(next) }
