...
Administrators may permanently delete a member from the role/action menu after an explicit confirmation. The server prevents self-deletion and performs application-profile and Neon Auth identity deletion in one database transaction. Neon Auth sessions and account records and OSai project memberships are removed through foreign-key cascades. The deletion leaves a minimal audit event keyed by the former immutable auth user ID without copying the member's email or display name into audit metadata. If the member created or last updated project or Legal records, deletion is rejected until those records are reassigned; protected business and Legal history is never silently removed.

The directory's member-facing Role and Status labels are derived from both the application profile and durable General MNDA envelope state. A site-approved non-administrator without a verified completed production General MNDA is shown as **Site Member** with **Pending MNDA** status. A completed DocuSign demo envelope never satisfies this gate. After verified production completion, the member is shown as **Pending Approval** until the separate final administrator-approval state is implemented.
