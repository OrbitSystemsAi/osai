...
function ProjectsPage({ isAdmin }: { isAdmin: boolean }) {
  ...
  if (isAdmin&&adminEditingProjectId) return <AdminProjectDetailPage projectId={adminEditingProjectId} onBack={closeProject} onProjectSaved={syncAdminProject}/>
...
type AdminProjectDetail = AdminProject & { image_url: string; user_goal: number; user_actual: number; cost_budget: number; cost_actual: number; adoption_rate: number; forecast_penetration: number; milestones: Milestone[]; tasks: ProjectTask[]; created_at?: string }
...
function AdminProjectDetailPage({ projectId, onBack, onProjectSaved }: { projectId: string; onBack: () => void; onProjectSaved?: (project: AdminProjectDetail) => void }) {
  ...
  const save = async () => { try { setMessage('Saving…'); const normalized = { ...draft, name: (draft.name.trim() || 'Project Title').slice(0, PROJECT_TITLE_MAX), description: draft.description.trim().slice(0, PROJECT_DESCRIPTION_MAX) }; await adminRequest(`/api/admin/projects/${projectId}`, { method: 'PATCH', body: JSON.stringify({ name: normalized.name, slug: normalized.slug, description: normalized.description, status: normalized.status, accessLevel: normalized.access_level }) }); await adminRequest(`/api/admin/projects/${projectId}`, { method: 'PATCH', body: JSON.stringify({ dashboard: { imageUrl: normalized.image_url, userGoal: normalized.user_goal, costBudget: normalized.cost_budget, costActual: normalized.cost_actual, adoptionRate: normalized.adoption_rate, forecastPenetration: normalized.forecast_penetration, milestones: normalized.milestones, tasks: normalized.tasks } }) }); setDraft(normalized); setProject(normalized); onProjectSaved?.(normalized); setEditingPart(null); setMessage('') } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not save the project dashboard.') } }
  const saveProjectImage = async (imageUrl: string) => { try { setMessage('Saving…'); const next = { ...draft, image_url:imageUrl }; await adminRequest(`/api/admin/projects/${projectId}`, { method:'PATCH', body:JSON.stringify({ dashboard:{ imageUrl, userGoal:next.user_goal, costBudget:next.cost_budget, costActual:next.cost_actual, adoptionRate:next.adoption_rate, forecastPenetration:next.forecast_penetration, milestones:next.milestones, tasks:next.tasks } }) }); setDraft(next); setProject(next); onProjectSaved?.(next); setEditingPart(null); setMessage('') } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not save the project image.') } }
  const saveTimeline = async (milestones: Milestone[]) => { try { const ordered = [...milestones].sort((a,b)=>a.date.localeCompare(b.date)); const next = { ...draft, milestones:ordered }; setDraft(next); await adminRequest(`/api/admin/projects/${projectId}`, { method:'PATCH', body:JSON.stringify({ dashboard:{ imageUrl:next.image_url, userGoal:next.user_goal, costBudget:next.cost_budget, costActual:next.cost_actual, adoptionRate:next.adoption_rate, forecastPenetration:next.forecast_penetration, milestones:ordered, tasks:next.tasks } }) }); setProject(next); onProjectSaved?.(next); setMessage('') } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not save the project timeline.') } }
